...
object Shades {
    ...
    fun of(hue: Float, chroma: Float): IntArray {
        ...
        shades[0] = ColorUtils.CAMToColor(hue, 40f.coerceAtMost(chrom), 99f)
        shades[1] = ColorUtils.CAMToColor(hue, 40f.coerceAtMost(chrom), 95f)
        for (i in 2..11) {
            val lStar = if (i == 6) MIDDLE_LSTAR else (100 - 10 * (i - 1)).toFloat()
            if (lStar >= 90) {
                chrom = 40f.coerceAtMost(chrom)
            }
            shades[i] = ColorUtils.CAMToColor(hue, chrom, lStar)
        }
        return shades
    }
}