...
package org.slimroms.systemui.monet
    ...
    fun getHueRotation(sourceHue: Float, hueAndRotations: List<Pair<Int, Int>>): Double {
        ...
        for (i in 0..hueAndRotations.size - 2) {
            ...
            if (thisHue <= sanitizedSourceHue && sanitizedSourceHue < nextHue) {
                return SlimColorScheme.wrapDegreesDouble(sanitizedSourceHue.toDouble() +
...
internal class HueAdd(val amountDegrees: Double) : Hue {
    override fun get(sourceColor: Cam): Double {
        return SlimColorScheme.wrapDegreesDouble(sourceColor.hue.toDouble() + amountDegrees)
    }
}
...
enum class SlimStyle(internal val coreSpec: CoreSpec) {
    SPRITZ(CoreSpec(
...
class SlimColorScheme(
    @ColorInt val accent: Int,
    @ColorInt val neutral: Int,
    val darkTheme: Boolean,
    styleOriginal: Style = Style.TONAL_SPOT
) {
    ...
    constructor(@ColorInt seed: Int, darkTheme: Boolean):
            this(seed, seed, darkTheme, Style.TONAL_SPOT)
    constructor(@ColorInt seed: Int, darkTheme: Boolean, style: Style = Style.TONAL_SPOT): this(seed, seed, darkTheme, style)
    @JvmOverloads
    constructor(
    ...
    init {
        style = when (styleOriginal) {
            Style.SPRITZ -> SlimStyle.SPRITZ
            Style.TONAL_SPOT -> SlimStyle.TONAL_SPOT
            Style.VIBRANT -> SlimStyle.VIBRANT
            Style.EXPRESSIVE -> SlimStyle.EXPRESSIVE
            Style.RAINBOW -> SlimStyle.RAINBOW
            Style.FRUIT_SALAD -> SlimStyle.FRUIT_SALAD
            Style.CONTENT -> SlimStyle.CONTENT
            Style.MONOCHROMATIC -> SlimStyle.MONOCHROMATIC
            else -> SlimStyle.VIBRANT
        }
        val proposedAccentCam = Cam.fromInt(accent)
        val proposedNeutralCam = Cam.fromInt(neutral)
        val accentArgb = if (accent == Color.TRANSPARENT) {
            GOOGLE_BLUE
        } else if (style != SlimStyle.CONTENT && proposedAccentCam.chroma < 5) {
            GOOGLE_BLUE
        } else {
            accent
        }
        val neutralArgb = if (neutral == Color.TRANSPARENT) {
            GOOGLE_BLUE
        } else if (style != SlimStyle.CONTENT && proposedNeutralCam.chroma < 5) {
            GOOGLE_BLUE
        } else {
            neutral
        }

        val camAccent = Cam.fromInt(accentArgb)
        val camNeutral = Cam.fromInt(neutralArgb)
        accent1 = style.coreSpec.a1.shades(camAccent)
        accent2 = style.coreSpec.a2.shades(camAccent)
        accent3 = style.coreSpec.a3.shades(camAccent)
        neutral1 = style.coreSpec.n1.shades(camNeutral)
        neutral2 = style.coreSpec.n2.shades(camNeutral)
    }
    override fun toString(): String {
        return "ColorScheme {\n" +
                "  accent color: ${stringForColor(accent)}\n" +
                "  neutral color: ${stringForColor(neutral)}\n" +
                "  style: $style\n" +
                "  palettes: \n" +
